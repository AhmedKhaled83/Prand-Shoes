...
function prand_Woman_prodect({ navigation, route }) {

  const [Prand_shoes, setPrand_shoes] = useState(route.params.prodact_Woman)
  ...
  const searchFun = (keySearch) => {
    ...
    for (var counter = 0; counter < list.length; counter++) {
      if (list[counter].prodact_name.toLowerCase().includes(keySearch.trim().toLowerCase())) {
          ...
          <FlatList data={Prand_shoes}
            ...
            renderItem={({ item, index }) => (
              ...
              <View style={{
                ...
                <TouchableOpacity
                  onPress={() => {
                    navigation.navigate("prand_prodect", {
                      Prand_shoes: item.prodact
                    })
                  }}

                  style={style.screen_image}>

                  <Image source={item.image} resizeMode="cover" style={{ width: width * 0.42, height: height * 0.2, borderRadius: 20 }} />

                </TouchableOpacity>


                <Text style={{ fontSize: 15, fontWeight: "bold", color: "#000" }}>{item.prodact_name}</Text>
...
export default prand_Woman_prodect;
